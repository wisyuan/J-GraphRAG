...
- 嵌入解耦实验已闭环（Phase 56/56b，2026-07-26）：**bge-m3 确认留任为架构必需件**——ws 向量替代 bge 种子全形态证伪（文体陷阱 + 词表错层），总观层面匹配必须嵌入模型；J-Lens 向量有效域=图内接地/消歧/多跳判别。EmbedProvider 协议保留（用户可换嵌入模型），但不再存在"去嵌入"路线。
